...
def log(message):
    ...
    conn = sqlite3.connect(DATABASE_NAME)
    cursor = conn.cursor()
    cursor.execute(f"INSERT INTO logs VALUES ('{timestamp}', '{message}')")  # SQL injection risk
    conn.commit()
    conn.close()
...
# ============================
# CLI Interface (bad input handling)
# ============================
