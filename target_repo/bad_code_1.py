...
def Do_Everything_Func(inputData, flag_value):
    ...
    # Horribly inefficient custom bubble sort (O(N^2)) instead of sorted()
    for i in range(len(temp_list)):
        for j in range(len(temp_list) - 1):
            # Bug: Fails if 'age' is a string
    ...
    for x in temp_list:
        try:
            if x['status'] == 'active':
                # Bug: Fails if balance is a string, but the exception is silenced!
                new_bal = x['balance'] + 100 
                
                # Terrible variable naming
                processed_results.append({'n': x['name'], 'b': new_bal, 'a': x['age']})
        except Exception as e:
            pass # Silent failure: the worst kind of error handling

    # Manual, inefficient deduplication (O(N^2))
    final_output = []
    ...
    # Hardcoded I/O operations mixed with business logic
...
def complex_math_stuff(a, b):
    # Completely reinventing basic addition with a loop
    if type(a) == str:
        a = int(a)
    if type(b) == str:
        b = int(b)
...
if __name__ == "__main__":
    # Test data filled with type inconsistencies waiting to crash the program
    dummy_data = [
        {'name': 'alice', 'age': 30, 'balance': 150, 'status': 'active'},
        {'name': 'bob', 'age': 25, 'balance': '200', 'status': 'active'},     # String balance!
        {'name': 'charlie', 'age': 35, 'balance': 300, 'status': 'inactive'},
        {'name': 'alice', 'age': 30, 'balance': 150, 'status': 'active'},     # Duplicate
        {'name': 'dave', 'age': 'forty', 'balance': 500, 'status': 'active'}  # String age!
    ]
